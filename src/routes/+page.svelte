<script lang="ts">
	...
	import route from '$img/ruta.png';
	...
	async function fetchData(init: number | any | undefined) {
		const response = await fetch('/api/lastone');
		if (response.ok) {
	...
	async function fetchDataTen() {
		...
		const response = await fetch('/api/lastten');
		if (response.ok) {
	...
	async function off_Auto(imei: string | any) {
		...
		const url = new URL('http://localhost:3030/off_auto');
				...
				<div class="flex items-center flex-col">
					...
					<div class="w-full flex mb-5">
						...
						<button
							...
							><img src={route} width="20" alt="pin" /></button
						>
						<button
							class="flex-grow hover:bg-slate-300 border flex justify-center text-white rounded-r p-2"
